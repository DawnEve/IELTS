...
def  add_statistic_routes(app):
    ...
    def statistics():
        ...
        twoWeek=int(time.mktime(  time.strptime(str(twoWeek), '%Y-%m-%d')   ))
        #24h内
        ...
        #
        print('twoWeek=',twoWeek)
        week21=mydb.query('select count(*) from word_ms where add_time>%d and add_time<%d;'% (twoWeek,oneWeek) );
        week22=mydb.query('select count(*) from word_unknown where add_time>%d and add_time<%d;'% (twoWeek,oneWeek) );
        week23=mydb.query('select count(*) from word_searched where add_time>%d and add_time<%d;'% (twoWeek,oneWeek) );
        #
        #
        ...
        sentenceWeek2=mydb.query('select count(*) from sentence_dawn where add_time>%d and add_time<%d;'% (twoWeek,oneWeek) );
        #返回值
        return cors({
